...
function generateBlenderImportScript(jsonFilePath: string, nodeTreeName: string, clearExisting: boolean = true, saveBlendFile: string = ""): string {
    // 根据操作系统格式化路径
...
server.tool(
    "import-bvtk-config",
    "Import JSON configuration to Blender BVtkNodes with automatic blend file saving",
    {
...
server.tool(
    ...
    "Generate a standalone Python script for importing JSON to Blender (can be run manually)",
    {
...
server.tool(
    "analyze-bvtk-config",
    "Analyze JSON configuration structure for BVtkNodes (looks for file on user's desktop)",
    {
    ...
    async ({ json_filename }) => {
        try {
            ...
            if (configData.nodes && Array.isArray(configData.nodes)) {
                ...
                if (nodeCount > 0) {
                    // 分析节点类型
                    const nodeTypes = [...new Set(configData.nodes.map((node: { bl_idname: any; type: any; }) => node.bl_idname || node.type || '未知'))].sort();
                    ...
                    nodeTypes.forEach(type => {
                        const count = configData.nodes.filter((node: { bl_idname: any; type: any; }) => (node.bl_idname || node.type) === type).length;
                        result += `  - ${type}: ${count}个\n`;
                    });
                    result += `\n`;
                }
...
server.tool(
    ...
    "List all JSON files on user's desktop",
    {},
    async () => {
        try {
            const desktopPath = getUserDesktopPath();
